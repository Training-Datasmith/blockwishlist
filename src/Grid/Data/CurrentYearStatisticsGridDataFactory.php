...
class Current_Year_Statistics_Grid_Data_Factory extends Base_Grid_Data_Factory implements Grid_Data_Factory_Interface
{
    // 1 month
    public const CACHE_LIFETIME_SECONDS = 2629746;
    public function get_data(Search_Criteria_Interface $search_criteria)
    {
        if ($this->cache->contains(self::CACHE_KEY_STATS_CURRENT_YEAR . $this->shop_id)) {
            $results = $this->cache->fetch(self::CACHE_KEY_STATS_CURRENT_YEAR . $this->shop_id);
        } else {
            $results = $this->calculator->compute_stats_for('currentYear');
            $this->cache->save(self::CACHE_KEY_STATS_CURRENT_YEAR . $this->shop_id, $results, self::CACHE_LIFETIME_SECONDS);
        }
